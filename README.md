...
The Android app also has a Settings dialog where testers can change and validate the API URL, log out, inspect cache size, and clear the local pack cache without rebuilding the APK. Network, HTTP, and invalid URL failures are surfaced as friendly status messages.
