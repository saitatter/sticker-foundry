...
- Duplicate sticker uploads are blocked with perceptual hash detection inside each pack.
...
- Web editor outline and drop-shadow toggles for sticker prep.
- Transparent checkerboard previews in sticker and image-edit surfaces.
- Batch upload normalization to a square transparent canvas.
...
The Android app also has a Settings dialog where testers can see the current account, change and validate the API URL, log out, inspect cache size, open import troubleshooting, and clear the local cache without rebuilding the APK. Each pack can also be resynced manually or removed from the local cache from its card. Network, HTTP, and invalid URL failures are surfaced as friendly status messages.
...
Android signing preparation is documented in [docs/ANDROID_SIGNING.md](docs/ANDROID_SIGNING.md).
